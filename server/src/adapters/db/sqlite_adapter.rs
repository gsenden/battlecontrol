...
const ENSURE_SCHEMA_VERSIONS_TABLE_SQL: &str =
...
impl SqliteAdapter {
    ...
    pub fn execute_with_params(&self, sql: &str, params: &[&dyn rusqlite::types::ToSql]) -> Result<(), String> {
        let conn = self.connection.lock()
            .map_err(|e| e.to_string())?;
    ...
    pub fn query_with_params(&self, sql: &str, params: &[&dyn rusqlite::types::ToSql]) -> Result<Vec<Row>, String> {
        let conn = self.connection.lock()
            .map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn execute_and_query() {
        ...
        adapter.execute("CREATE TABLE test (id INTEGER, name TEXT)").unwrap();
        adapter.execute_with_params(
    ...
    fn query_with_params() {
        ...
        let rows = adapter.query_with_params(

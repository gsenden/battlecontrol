...
output += `export type Language = ${langCodes.map((l) => `'${l}'`).join(' | ')};\n\n`;

output += `export type I18nKey = ${keys.map((k) => `'${k}'`).join(' | ')};\n\n`;
...
console.log(`Generated ${outFile} with ${keys.length} keys in ${langCodes.length} languages`);
